...
- `bool BeforeCatch` : `#!cpp catch` 之前的括号
...
- `bool BeforeWhile` : `#!cpp while` 前的括号
...
- `bool SplitEmptyNamespace` : 如果为 `#!yml false`，则空的命名空间主体可以放在一行上。该选项仅在名称空间的左大括号已经被包装时使用，即设置了 `AfterNamespace` 大括号包装模式。
